...
class BoundingBox:
    ...
    def __post_init__(self):
        ...
        if self.width < 0 or self.height < 0:
            raise ValueError(f"Invalid bounding box: ({self.x1}, {self.y1}) to ({self.x2}, {self.y2})")
...
class PoseData:
    ...
    def get_keypoint(self, name: str) -> Keypoint:
        ...
        if name not in KEYPOINT_INDICES:
            raise KeyError(f"Invalid keypoint name: {name}. Valid names: {list(KEYPOINT_INDICES.keys())}")
