...
from ..tasks.validators.squat_validator import SquatValidator
from ..tasks.validators.pushup_validator import PushupValidator
...
class TaskExecutingState(GameState):
    ...
    def enter(self, context: GameContext) -> None:
        if context.current_task:
            ...
            # TODO: Use a factory or registry in the future
            ex_name = context.current_task.exercise.name_en
            if ex_name == "squat":
                context.current_task.validator = SquatValidator()
            elif ex_name == "pushup":
                context.current_task.validator = PushupValidator()
            else:
                # Fallback or error for unknown exercises
                # For MVP, we might just default to Squat or log warning
                context.current_task.validator = None
                self._feedback = f"尚未支援此運動: {ex_name}"
