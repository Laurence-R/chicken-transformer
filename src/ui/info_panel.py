...
class InfoPanel:
    ...
    def __init__(self, x: int, y: int, width: int, height: int):
        ...
        # Fallback to default system font if specific ones aren't found
        self.font_large = pygame.font.SysFont("Microsoft JhengHei", 32) 
        if not self.font_large.get_height(): # Check if font loaded correctly-ish (pygame doesn't raise error usually)
             self.font_large = pygame.font.SysFont("WenQuanYi Zen Hei", 32) # Linux common
        
        self.font_medium = pygame.font.SysFont("Microsoft JhengHei", 24)
        if not self.font_medium.get_height():
             self.font_medium = pygame.font.SysFont("WenQuanYi Zen Hei", 24)

        self.font_small = pygame.font.SysFont("Microsoft JhengHei", 18)
        if not self.font_small.get_height():
             self.font_small = pygame.font.SysFont("WenQuanYi Zen Hei", 18)
    ...
    def update(self, context: GameContext, fps: float):
        ...
        if context.current_task:
            ...
            # Target
            text = self.font_small.render(f"目標: {task.reps_per_set} 次 x {task.total_sets} 組", True, (200, 200, 200))
            self.surface.blit(text, (padding, y_offset))
            y_offset += 25
            ...
            if context.current_task:
                ...
                current_set_display = task.current_sets + 1 if task.current_sets < task.target_sets else task.target_sets
                ...
                bar_height = 20
                pygame.draw.rect(self.surface, (100, 100, 100), (padding, y_offset, bar_width, bar_height))
                
                if task.target_reps > 0:
                    pct = min(1.0, task.current_reps / task.target_reps)
                    fill_width = int(bar_width * pct)
                    pygame.draw.rect(self.surface, (0, 255, 0), (padding, y_offset, fill_width, bar_height))
                y_offset += 30
