...
class TensorRTPoseDetector(PoseDetector):
    ...
    def __init__(self, engine_path: str, confidence_threshold: float = 0.5, nms_threshold: float = 0.45):
    ...
    def initialize(self) -> bool:
        ...
        if trt is None or cuda is None:
            print("Error: TensorRT or PyCUDA not installed. Cannot initialize TensorRTPoseDetector.")
            return False
    ...
    def _preprocess(self, img: np.ndarray) -> Tuple[np.ndarray, Tuple[float, float], Tuple[float, float]]:
    ...
    def _postprocess(self, output, ratio, dwdh, orig_shape) -> Optional[PoseData]:
        ...
        import cv2
        indices = cv2.dnn.NMSBoxes(boxes_xyxy.tolist(), scores.tolist(), self.conf_thres, self.nms_thres)
        
        if len(indices) == 0:
            return None
            
        # Pick the best detection (highest confidence)
        # For multi-person, we might want to return list, but PoseData is single person for now
        idx = indices[0] # indices is usually a list of indices
        ...
        coco_names = list(KEYPOINT_INDICES.keys())
