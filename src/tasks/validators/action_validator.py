...
class ActionValidator(ABC):
    ...
    def __init__(
        ...
        if not 0.05 <= tolerance_distance_ratio <= 0.20:
            raise ValueError(f"Distance tolerance must be in [0.05, 0.20]: {tolerance_distance_ratio}")
        if not 0.3 <= min_confidence_threshold <= 0.7:
            raise ValueError(f"Confidence threshold must be in [0.3, 0.7]: {min_confidence_threshold}")
