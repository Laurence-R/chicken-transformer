...
class MockPoseDetector(PoseDetector):
    ...
    def __init__(self, mode: str = "standing", noise_level: float = 0.0, confidence: float = 0.9):
    ...
    def get_model_info(self) -> dict:
        ...
        avg_inference_time = (
            (elapsed_time / self.frame_count * 1000) if self.frame_count > 0 else 0.0
        )
    ...
    def _add_noise(self, keypoints: list[Keypoint]) -> list[Keypoint]:
        ...
        for kp in keypoints:
            ...
            noisy_keypoints.append(
                Keypoint(
                    x=max(0, kp.x + noise_x), y=max(0, kp.y + noise_y), confidence=kp.confidence
                )
            )
